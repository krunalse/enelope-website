...
import type { ServiceRow, ServiceTranslations } from "@/lib/supabase/queries";
...
interface ServiceFormProps {
  ...
  service?: (ServiceRow & { translations: ServiceTranslations }) | null;
  submitLabel: string;
}
    ...
    <form action={action} className="space-y-6" encType="multipart/form-data">
      <div>
        ...
        <input
          id="slug"
          name="slug"
          placeholder="auto-generated from the English title if left blank"
          defaultValue={service?.slug}
          className={inputClass}
        />
        <p className="mt-1.5 text-xs text-ink-soft dark:text-white/40">
          The slug stays the same across every language.
        </p>
      </div>

      <div>
        <p className={labelClass}>Title &amp; description</p>
        <LocaleFieldTabs
            ...
            LOCALES.map((locale) => {
              ...
              return [
                ...
                <div className="space-y-5" key={locale}>
                  <div>
                    <label htmlFor={`title_${locale}`} className={labelClass}>
                      Title
                    </label>
                    <input
                      id={`title_${locale}`}
                  ...
                  <div>
                    <label htmlFor={`short_description_${locale}`} className={labelClass}>
                      Short description
                    </label>
                    <textarea
                  ...
                  <div>
                    <label htmlFor={`full_description_${locale}`} className={labelClass}>
                      Full description
                    </label>
                    <textarea
                      id={`full_description_${locale}`}
                      name={`full_description_${locale}`}
                      required={locale === "en"}
                      rows={5}
                      defaultValue={t?.full_description}
                      className={inputClass}
                    />
                  </div>
                </div>,
